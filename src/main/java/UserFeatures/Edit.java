 ...
 * Key behavior:
 * <ul>
 * <li>{@link #applyEdit(Edit, boolean, boolean)} is the single source of truth for applying/undoing
 * edits.</li>
 * <li>Proposal mode updates only the temporary balance (sandbox) and does not mutate real
 * budgets.</li>
...
public class Edit {
   ...
   *
   * <p>
   * In proposal mode, this is treated as a sandbox balance used to validate increases against the
   ...
   *
   * <p>
   * During proposal creation, edits are accumulated here and then written to a proposal file.
   * </p>
   */
   ...
   * <li>If {@code isProposal == false}, the target ministry budget is updated and autosave may
   * occur.</li>
   * </ul>
   *
  ...
  public static void applyEdit(Edit edit, boolean undo, boolean isProposal) {
    ...
    if (m == null) {
      throw new IllegalArgumentException("Ministry not found: " + edit.getName());
    }
  ...
  public void zerobalance(boolean isProposal) {
    System.out.println(
        "You have to decrease first a ministry's budget because you do not have available money ");
    ...
    System.out.println("The budget of the " + fromName + " is " + Ministry.getFormattedBudget(
        Ministry.budgetSearchByName(fromName, CreatingMinistries.ministries2026)));

    System.out.println("Enter amount to decrease:");
    double amt =
        validityAmount(Ministry.budgetSearchByName(fromName, CreatingMinistries.ministries2026));
  ...
  public void nonzerobalance(boolean isProposal) {
    ...
    do {
      ...
      System.out
          .println("Available money for Investment : " + Ministry.getFormattedBudget(balance));
  ...
  public String serialize() {
    return "EDIT|" + (name == null ? "" : name) + "|" + (change == null ? "" : change) + "|"
        + amount + "|" + (changeType == null ? "" : changeType);
  }
