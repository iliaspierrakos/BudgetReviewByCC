...
/**
 * The {@code BulkEdit} class handles bulk operations on ministry budgets. It allows applying
 * changes to all ministries or selected ones,
 */
public class BulkEdit {
  ...
  /**
   * Applies a percentage change to ALL ministries with preview. User can specify positive
   * (increase) or negative (decrease) percentages. Shows a detailed preview before confirming
   * changes. Updates the Edit.balance accordingly.
   */
  private void percentageChangeAll(int x, int choice) {
    if (x == 1) {
      ...
      System.out
          .println("Now, your available balance is " + Ministry.getFormattedBudget(Edit.balance));
  ...
  /**
   * Applies a fixed amount change to ALL ministries with preview. User can specify positive (add)
   * or negative (subtract) amounts. Validates that no ministry budget becomes negative. Updates the
   * Edit.balance accordingly.
   */
  private void fixedAmountChangeAll(int x, int choice) {
    if (x == 1) {
      ...
      System.out.println(
          "Great. Now the available money is " + Ministry.getFormattedBudget(Edit.balance));
    ...
    if (totalChange > Edit.balance) {
      ...
      if (confirm.equalsIgnoreCase("yes")) {
        ...
        System.out.println(
            "Available money for Investment: " + Ministry.getFormattedBudget(Edit.balance));
        System.out.println("All budgets updated successfully!");
      } else {
        System.out.println("Operation cancelled.");
      }
  ...
  /**
   * Applies changes to SELECTED ministries. User selects specific ministries by entering their
   * numbers (comma-separated). Then chooses between percentage or fixed amount change. Shows
   * preview and updates Edit.balance accordingly.
   */
  private void selectedMinistriesEdit(int x, int choice) {
    if (x == 1) {
      ...
      System.out.println("Successful decrease. Available money now is "
          + Ministry.getFormattedBudget(Edit.balance) + ".");
  ...
  private void showBeforeAfterPreview(double percentage, ArrayList<Integer> selectedIndices) {
    ...
    // COLUMN HEADERS
    TableUtils.appendTableRowCustom(sb, 55, 25, "MINISTRY", "CURRENT BUDGET", "NEW BUDGET",
        "CHANGE");
    TableUtils.appendSeparator(sb, 140, '-');
    ...
    for (int i = 0; i < CreatingMinistries.ministries2026.length; i++) {
      ...
      if (m != null) {
        ...
        if (selectedIndices == null || selectedIndices.contains(i)) {
          ...
          TableUtils.appendTableRowCustom(sb, 55, 25, m.getMinistryName(),
              Ministry.getFormattedBudget(oldBudget), Ministry.getFormattedBudget(newBudget),
    ...
    // FOOTER
    TableUtils.appendSeparator(sb, 140, '-');
    TableUtils.appendTableRowCustom(sb, 55, 25, "TOTAL",
  ...
  /**
   * Displays a before/after preview for fixed amount changes. Shows current budget, new budget, and
   * the fixed change amount for each ministry.
   *
  ...
  private void showFixedAmountPreview(double amount, ArrayList<Integer> selectedIndices) {
    ...
    TableUtils.appendTitle(sb,
        "PREVIEW - BUDGET CHANGES (Fixed Amount: " + Ministry.getFormattedBudget(amount) + ")",
        140);
    TableUtils.appendSeparator(sb, 140, '=');
    ...
    // FOOTER
    TableUtils.appendSeparator(sb, 140, '-');
    TableUtils.appendTableRowCustom(sb, 55, 25, "TOTAL (" + affectedCount + " ministries)",
        Ministry.getFormattedBudget(totalCurrentBudget),
        Ministry.getFormattedBudget(totalNewBudget),
  ...
  /**
   * Calculates the total budget change resulting from a percentage operation. Used to update the
   * Edit.balance appropriately.
   *
  ...
  public void executionSelectedFixed(double amount, ArrayList<Integer> selectedIndices) {
    ...
    if (confirm.equalsIgnoreCase("yes")) {
      ...
      System.out
          .println("Available money for Investment: " + Ministry.getFormattedBudget(Edit.balance));
      System.out.println("Selected ministries updated!");
    } else {
      System.out.println("Operation cancelled.");
    }
  ...
  /**
   * GUI: Apply change to SELECTED ministries. - No prompts - Validates balance + negative budgets -
   * Updates Edit.balance - Records history (same as your apply* methods)
   */
  public BulkEditResult applySelectedGui(ArrayList<Integer> selectedIndices, ChangeMode mode,
      double value, ChangeType type) {
    ...
    if (type == ChangeType.DECREASE) {
      signedValue = -Math.abs(value);
    } else {
      signedValue = Math.abs(value);
    }
    ...
    if (mode == ChangeMode.FIXED) {
      ...
      for (int idx : selectedIndices) {
        ...
        if (m != null && m.getBudget() + signedValue < 0) {
          return new BulkEditResult(false,
              "This change would make budget negative for: " + m.getMinistryName(), 0);
        }
      }
  ...
  /**
   * GUI: Preview rows for selected ministries (before/after), without applying. Useful to show in a
   * TableView preview.
   */
  public ArrayList<PreviewRow> previewSelectedGui(ArrayList<Integer> selectedIndices,
      ChangeMode mode, double value, ChangeType type) {
    ...
    for (int idx : selectedIndices) {
      ...
      rows.add(new PreviewRow(m.getMinistryName(), oldB, newB, newB - oldB));
    }
    return rows;
  }
  ...
  public static class PreviewRow {
    ...
    public String getCurrentBudgetText() {
      return Ministry.getFormattedBudget(currentBudget);
    }

    public String getNewBudgetText() {
      return Ministry.getFormattedBudget(newBudget);
    }

    public String getChangeText() {
      return Ministry.getFormattedBudget(change);
    }
