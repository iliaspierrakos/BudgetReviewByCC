...
/**
 * The {@code TaxReceipt} class is responsible for calculating an individual's tax based on income,
 * number of children, and age, and for distributing that tax proportionally across government
 * ministries.
 *
 * <p>
 * The generated result is primarily intended for GUI presentation, providing both numeric values
 * and formatted text suitable for display.
 * </p>
 *
 * <p>
 * This class contains two helper data classes ({@link TaxRow} and {@link TaxResult}) as well as
 * utility methods for tax calculation and distribution.
 * </p>
 *
 */
public class TaxReceipt {
  ...
  /**
   * Represents a single row in the tax receipt table. Each row corresponds to a government ministry
   * and shows the user's contribution to that ministry.
   */
  ...
  /**
   * Encapsulates the complete tax calculation result for a user. This includes personal data, total
   * tax, formatted values, and the detailed distribution across ministries.
   */
   ...
   *
   * <p>
   * The method validates input values, calculates the total tax, and distributes it proportionally
   * according to each ministry's budget.
   * </p>
   *
   ...
   *
   * @throws IllegalArgumentException if income or kids are negative, or if age is below 18
  ...
  public static TaxResult generateForGui(double income, int kids, int age) {
    ...
    if (kids < 0) {
      throw new IllegalArgumentException("Kids cannot be negative.");
    }
    if (age < 18) {
  ...
  /**
   * Calculates the tax amount based on progressive tax brackets, age-related exemptions, and
   * child-related reductions.
   *
   * <p>
   * This method implements the tax rules defined for the project and is not exposed publicly.
   * </p>
   *
   * @param income user's income
   * @param kids number of children
