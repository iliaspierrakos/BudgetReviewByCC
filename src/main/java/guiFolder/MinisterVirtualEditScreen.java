...
public class MinisterVirtualEditScreen {
  ...
  public void show(Stage stage) {
    ...
    Label subtitle =
        new Label("Create draft edits (no official changes) and submit to Prime Minister.");
    ...
    VBox simpleEdit =
        actionCard("Simple Draft Edit", "Edit one ministry using a fixed amount (draft).",
            "/icons/wand.png", () -> openSimpleDraftEditDialog(stage, balanceChip, editsChip));
    ...
    VBox history = actionCard("Draft History", "Review and undo your draft changes.",
        "/icons/history.png", () -> new DraftHistoryScreen(user, userManager).show(stage));
    history.getStyleClass().add("primary-action");

    VBox send =
        actionCard("Send to Prime Minister", "Export draft proposal into ProposalsFromMinisters/.",
            "/icons/selected.png", () -> DraftProposalExporter.exportAndNotify(stage, user));
    send.getStyleClass().add("primary-action");

    VBox reset = actionCard("Reset Draft",
        "Discard all draft edits and restore from current official budgets.", "/icons/reset.png",
  ...
  private void openSimpleDraftEditDialog(Stage parentStage, Label balanceChip, Label editsChip) {
    ...
    Label subtitle =
        new Label("Select ministry, choose change type and enter amount (draft only).");
    subtitle.getStyleClass().add("subtitle");
    ...
    ministryBox.valueProperty().addListener((obs, o, n) -> {
      ...
      if (n != null) {
        ...
        if (idx >= 0 && DraftEditSession.getSandbox()[idx] != null) {
          ...
          currentBudgetLabel
              .setText("Current Draft Budget: " + Ministry.getFormattedBudget(budget));
        } else {
