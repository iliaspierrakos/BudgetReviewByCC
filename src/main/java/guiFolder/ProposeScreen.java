 ...
 *
 * Adds: - Reload Original Budget Review (loads Governor CSV baseline, then resets draft) - Reset
 * Draft (resets draft from currently loaded budgets, no file reload)
 */
public class ProposeScreen {
  ...
  public void show(Stage stage) {
    ...
    if (!(user instanceof MinistryMember)) {
      Alert a = new Alert(Alert.AlertType.ERROR,
          "Access denied: Only authorized users can create proposals.");
      applyTheme(a);
      a.showAndWait();
      return;
    }
    ...
    VBox simpleEdit =
        actionCard("Simple Draft Edit", "Edit one ministry using a fixed amount (draft).",
            "/icons/wand.png", () -> openSimpleDraftDialog(stage, balanceChip, countChip));
    simpleEdit.getStyleClass().add("primary-action");

    VBox bulkEdit = actionCard("Bulk Draft Edit", "Apply changes to multiple ministries (draft).",
        "/icons/bulk.png", () -> new BulkEditDraftScreen(user, userManager).show(stage));
    bulkEdit.getStyleClass().add("primary-action");

    VBox history = actionCard("Draft History", "Review and undo your draft edits.",
        "/icons/history.png", () -> new DraftHistoryScreen(user, userManager).show(stage));
    history.getStyleClass().add("primary-action");

    // NEW: Reload original baseline from Governor CSV
    VBox reloadOriginal = actionCard("Reload Original Budget Review",
        "Reload baseline (Governor CSV) and clear draft edits.", "/icons/reset.png",
    ...
    VBox resetDraft = actionCard("Reset Draft",
  ...
  private void reloadOriginalBudgetReview(Stage stage, Label balanceChip, Label countChip) {
    ...
    confirm.setContentText(
        "This will discard all draft edits and reload the original baseline (Governor CSV).");
  ...
  private void resetDraftFromCurrentLoadedBudgets(Stage stage, Label balanceChip, Label countChip) {
    ...
    confirm.setContentText(
        "This will clear draft edits and reset from currently loaded budgets (no reload).");
  ...
  /**
   * Internal helper: tries to load Governor CSV baseline (if exists), then resets session. If
   * showWarning=true, it will warn if baseline file missing.
   */
