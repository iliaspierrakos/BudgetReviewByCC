...
public class BulkEditScreen {
  ...
  private final boolean proposalMode;

  public BulkEditScreen(User user, UserManager userManager) {
    this(user, userManager, false);
  }

  public BulkEditScreen(User user, UserManager userManager, boolean proposalMode) {
    this.user = user;
  ...
  public void show(Stage stage) {
    ...
    // Draft/Proposal mode: only MinistryMember allowed
    if (proposalMode && !(user instanceof MinistryMember)) {
      Alert a = new Alert(Alert.AlertType.ERROR,
          "Access denied: Only Ministry Members can create proposal drafts.");
      applyTheme(a);
      a.showAndWait();
      return;
    }
    ...
    Label title = new Label(proposalMode ? "Bulk Draft Edit" : "Bulk Edit");
    title.getStyleClass().add("title");
    title.setStyle("-fx-effect: dropshadow(gaussian, rgba(212,175,55,0.14), 16, 0.20, 0, 0);");

    Label subtitle =
        new Label(proposalMode ? "Create draft edits (NOT persisted) for Prime Minister review."
            : "Apply changes to multiple ministries at once.");
    ...
    Label chip3 = new Label((proposalMode ? "Draft Balance: " : "Balance: ")
    ...
    VBox percentAllCard =
        actionCard("Percentage Change (All)", "Apply a percentage to all ministries with preview.",
            "/icons/percent.png", () -> showPercentageAllDialog(stage));

    VBox fixedAllCard = actionCard("Fixed Amount Change (All)",
        "Apply a fixed amount to all ministries with preview.", "/icons/exposure.png",
        () -> showFixedAllDialog(stage));

    VBox selectedCard =
        actionCard("Change Selected Ministries", "Select ministries, preview changes, then apply.",
            "/icons/selected.png", () -> showSelectedMinistriesDialog(stage));
    selectedCard.setStyle("-fx-border-color: rgba(212,175,55,0.18);");

    VBox backCard = actionCard("Back", "Return to previous screen.", "/icons/back.png", () -> {
      if (proposalMode) {
    ...
    Scene scene = new Scene(root, stage.getWidth() > 0 ? stage.getWidth() : 1120,
    ...
    applyScenePreserveWindow(stage, scene, proposalMode ? "Bulk Draft Edit" : "Bulk Edit");
  ...
  private VBox buildSidePanel() {
    Label t1 = new Label(proposalMode ? "How Draft Bulk Edit works" : "How Bulk Edit works");
    ...
    Label l5 = proposalMode ? new Label("• Draft mode does NOT save changes to file")
        : new Label("• Normal mode saves changes to budgets");
  ...
  private void showPercentageAllDialog(Stage parent) {
    ...
    Label balance = new Label((proposalMode ? "Draft Balance: " : "Available Balance: ")
        + Ministry.getFormattedBudget(Edit.balance));
    ...
    Button applyBtn = new Button(proposalMode ? "Apply Draft" : "Apply");
    ...
    previewBtn.setOnAction(e -> {
      ...
      if (!isValidPositiveNumber(txt)) {
        error.setText("Please enter a valid percentage.");
        return;
      }

      double pct = Double.parseDouble(txt);
      if (pct <= 0) {
        error.setText("Percentage must be positive.");
        return;
      }

      if (typeGroup.getSelectedToggle() == decBtn) {
        pct = -pct;
      }
      if (pct <= -100) {
      ...
      for (Ministry m : CreatingMinistries.ministries2026) {
        ...
        rows.add(new PreviewRow(m.getMinistryName(), Ministry.getFormattedBudget(oldB),
            Ministry.getFormattedBudget(newB), formatDelta(newB - oldB)));
      }
    ...
    applyBtn.setOnAction(e -> {
      ...
      for (Ministry m : CreatingMinistries.ministries2026) {
        ...
        if (newB < 0) {
          error.setText("Would create negative budget (unexpected).");
          return;
        }
      }
      ...
      for (Ministry m : CreatingMinistries.ministries2026) {
        ...
        // IMPORTANT: proposalMode => draft flag true
        Edit.applyEdit(editObj, false, proposalMode);
      }

      // Persist ONLY if not proposal/draft mode
      if (!proposalMode) {
        UserBudgetPersistence.saveUserBudgets(user, CreatingMinistries.ministries2026, 2026);
      }

      balance.setText((proposalMode ? "Draft Balance: " : "Available Balance: ")
          + Ministry.getFormattedBudget(Edit.balance));
      dialog.close();
    ...
    card.getChildren().addAll(title, hint, typeRow, percentField, balance, info, error, actions,
        table);
  ...
  private void showFixedAllDialog(Stage parent) {
    ...
    card.getChildren().addAll(title, hint, typeRow, amountField, balance, info, error, actions,
        table);
  ...
  private void showSelectedMinistriesDialog(Stage parent) {
    ...
    for (int i = 0; i < CreatingMinistries.ministries2026.length; i++) {
      ...
      pickRows.add(
          new MinistryPickRow(i, m.getMinistryName(), Ministry.getFormattedBudget(m.getBudget())));
    }
    ...
    Button applyBtn = new Button(proposalMode ? "Apply Draft" : "Apply Selected");
    applyBtn.getStyleClass().addAll("button", "primary");
    applyBtn.setDisable(true);
    ...
    Runnable doPreview = () -> {
      ...
      if (!isValidPositiveNumber(txt)) {
        error.setText("Please enter a valid value.");
        return;
      }

      double value = Double.parseDouble(txt);
      if (value <= 0) {
        error.setText("Value must be positive.");
        return;
      }
      ...
      for (MinistryPickRow r : selected) {
        ...
        rows.add(new PreviewRow(m.getMinistryName(), Ministry.getFormattedBudget(oldB),
            Ministry.getFormattedBudget(newB), formatDelta(delta)));
      }
    ...
    VBox controlsCard =
        new VBox(12, new Label("Controls"), new Separator(), new Label("Mode"), modeRow,
            new Label("Type"), typeRow, new Label("Value"), valueField, valueHint, error, actions);
