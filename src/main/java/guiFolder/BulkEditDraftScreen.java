 ...
 *
 * Rules: - Applies edits ONLY to DraftEditSession sandbox. - DraftEditSession is the single source
 * of truth for: - sandbox budgets - draft history - draft balance
 */
public class BulkEditDraftScreen {
  ...
  public void show(Stage stage) {
    ...
    Label subtitle =
        new Label("Create draft edits (NOT persisted). Submit only from Propose screen.");
    ...
    VBox percentAllCard = actionCard("Percentage Draft (All)",
        "Apply a percentage to all ministries with preview (draft).", "/icons/percent.png",
        () -> showPercentageAllDialog(stage, chip3));

    VBox fixedAllCard = actionCard("Fixed Amount Draft (All)",
        "Apply a fixed amount to all ministries with preview (draft).", "/icons/exposure.png",
        () -> showFixedAllDialog(stage, chip3));

    VBox selectedCard = actionCard("Draft Selected Ministries",
        "Select ministries, preview changes, then apply (draft).", "/icons/selected.png",
        () -> showSelectedMinistriesDialog(stage, chip3));
    selectedCard.setStyle("-fx-border-color: rgba(212,175,55,0.18);");

    VBox backCard = actionCard("Back", "Return to Propose screen (send from there).",
        "/icons/back.png", () -> new ProposeScreen(user, userManager).show(stage));
  ...
  private void showFixedAllDialog(Stage parent, Label balanceChip) {
    ...
    previewBtn.setOnAction(e -> {
      ...
      if (signed > 0) {
        ...
        double available = DraftEditSession.getDraftBalance();
        if (totalCost > available + 1e-9) {
          error.setText("Insufficient draft balance.");
          return;
        }
      }
