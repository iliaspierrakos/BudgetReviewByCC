...
public class RegisterScreen {
  ...
  public void show(Stage stage) {
    ...
    Supplier<User.Role> getRole = () -> {
      ...
      if (sel == ministryBtn) {
        return User.Role.MINISTRYMEMBER;
      }
    ...
    ministryBox.getItems().addAll("Ministry of Interior", "Ministry of Foreign Affairs",
    ...
    // ---- Actions ----
    registerButton.setOnAction(e -> attemptRegister(stage, usernameField, passwordField,
        ministryBox, getRole, errorLabel));

    backButton.setOnAction(e -> new StartMenuScreen(userManager).show(stage));

    // ---- Card layout ----
    VBox card = new VBox(12, logoFrame, title, subtitle, new Separator(), usernameField,
        passwordField, roleLabel, rolePicker, ministryBox, errorLabel, registerButton, backButton);
  ...
  private void attemptRegister(Stage stage, TextField usernameField, PasswordField passwordField,
      ComboBox<String> ministryBox, Supplier<User.Role> getRole, Label errorLabel) {
  ...
  private static void updateDisable(Button registerButton, TextField usernameField,
      PasswordField passwordField, Supplier<User.Role> getRole, ComboBox<String> ministryBox) {
    ...
    boolean ok =
        !u.isEmpty() && !p.isEmpty() && r != null && (!needsMinistry || selectedMinistry != null);
